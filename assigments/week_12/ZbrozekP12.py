...
def add_db():
    ...
    default = GeoPoint(
        row_id=1,
        lat=float(35.106766),
        lon = float(-106.629181), 
        city="Albuquerque",
        state= "New Mexico", 
        description="a magical place",
        distance=float(0))
    set_point_objects(tourList, default)
...
state_label = tk.Label(text="Enter State")
...
# Save to DB
tk.Button(text="Save to Database", command=add_db, bg="lightgreen").grid(row=1,column=2, padx=0, pady=5, sticky= 'W')
